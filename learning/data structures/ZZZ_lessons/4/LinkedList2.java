public class LinkedList2 {
	...
	Node reverse (Node node) {
		...
		while (current != null) {
			// set next to be that of current node's next
			next = current.next;
			// current's next is equal to previous (null, at least in the first iteration)
			current.next = previous;
			// previous is now evaluated to the current node
			previous = current;
			// 
			current = next;
		}
